...
function createPanelItem({ id, number = "", title, count }, activeView) {
  const button = createElement("button", {
    ...
    attributes: { type: "button", id: `topic-${id}`, "aria-pressed": String(activeView === id) },
  });
  if (number !== "") {
    button.append(createElement("span", { className: "side-panel__item-number", text: number }));
  }
  button.append(
...
function renderAllTopics(manifest, state) {
  ...
  for (const topic of topics) {
    ...
    sectionHeader.append(
      createElement("span", { className: "all-topics__number", text: formatNumber(topic.number) }),
